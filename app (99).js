...
function viewTrends(){
  ...
  function drawCm2(){
    portals.forEach(function(p,pi){
      ...
      if(!paneGroups['cm2'])paneGroups['cm2']=[];
      if(paneGroups['cm2'].indexOf('c-cm2-'+p)<0)paneGroups['cm2'].push('c-cm2-'+p);
      (function(ch,canvas){
        canvas.addEventListener('mousemove',function(e){
          var pts=ch.getElementsAtEventForMode(e,'index',{intersect:false},false);
          if(pts.length)syncPanes('cm2',ch,pts[0].index,e.clientX,e.clientY);
          else hideSyncTooltip('cm2');
        });
        canvas.addEventListener('mouseleave',function(){hideSyncTooltip('cm2');});
      })(charts['c-cm2-'+p],cv);
    });
  }
