...
function viewTrends(){
  ...
  const D={
    ...
    adsvCross:  cross(t=>t.netSales>0?+((t.ads+t.vis)/t.netSales*100).toFixed(1):null),
  ...
  const html=
    ...
    +secHead(2,'Marketing Spends','Ads + Visibility as % of Net Sales')
    +'<div class="g3 mb20">'
    +portals.map(p=>cCard(pLabel(p)+' · Ads+Vis % Net Sales','c-mkt-'+p,240)).join('')
    +'</div>'
    +'<div class="card mb20" style="padding:18px"><div style="font-size:12px;font-weight:700;color:var(--tx2);text-transform:uppercase;letter-spacing:.05em;margin-bottom:14px">Ads+Vis % Net Sales · All Portals</div><div style="position:relative;height:280px"><canvas id="c-mkt-cross"></canvas></div>'+pLeg+'</div>'
  ...
  var perPortal=${JSON.stringify(Object.fromEntries(portals.map(p=>[p,{
    ...
    gmv:    series(p,t=>Math.round(t.gmv/100000)),
  ...
  var adsvCross=${JSON.stringify(D.adsvCross)};
  var cm1Cross=${JSON.stringify(D.cm1Cross)};
  var cm2Cross=${JSON.stringify(D.cm2Cross)};

  var destroyed={};
  function mkChart(id,cfg){
    if(destroyed[id])try{destroyed[id].destroy();}catch(e){}
    var cv=document.getElementById(id);if(!cv)return;
    destroyed[id]=new Chart(cv,cfg);
  }

  var lineOpts=function(isPct,col){return{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:function(ctx){return ' '+(isPct?ctx.parsed.y.toFixed(1)+'%':(ctx.parsed.y).toFixed(1)+'L GMV');}}}},scales:{x:{grid:{display:false},ticks:{font:{size:10}}},y:{grid:{color:'#F0F5F5'},ticks:{font:{size:10},callback:function(v){return isPct?v+'%':v+'L';}}}}};};

  var barOpts=function(isPct){return{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:function(ctx){return ' '+ctx.dataset.label+': '+(isPct?ctx.parsed.y.toFixed(1)+'%':(ctx.parsed.y).toFixed(1)+'L');}}}},scales:{x:{grid:{display:false},ticks:{font:{size:10}}},y:{grid:{color:'#F0F5F5'},ticks:{font:{size:10},callback:function(v){return isPct?v+'%':v+'L';}}}},barPercentage:0.7,categoryPercentage:0.8};};

  // Single portal line charts
  portals.forEach(function(p){
    var pd=perPortal[p];
    if(!pd.months.length)return;
    var col=pColors[p];
    var alpha=col+'33';

    mkChart('c-gmv-'+p,{type:'line',data:{labels:pd.months,datasets:[{label:'GMV',data:pd.gmv,borderColor:col,backgroundColor:alpha,fill:true,tension:0.3,pointBackgroundColor:col,pointRadius:5,pointHoverRadius:7}]},options:lineOpts(false,col)});

    mkChart('c-mkt-'+p,{type:'line',data:{labels:pd.months,datasets:[{label:'Ads+Vis %',data:pd.adsvNet,borderColor:col,backgroundColor:alpha,fill:true,tension:0.3,pointBackgroundColor:col,pointRadius:5,pointHoverRadius:7}]},options:lineOpts(true,col)});

    mkChart('c-cm-'+p,{type:'line',data:{labels:pd.months,datasets:[
      {label:'CM1%',data:pd.cm1,borderColor:'#4C94D0',backgroundColor:'#4C94D022',fill:false,tension:0.3,pointBackgroundColor:'#4C94D0',pointRadius:5,pointHoverRadius:7},
      {label:'CM2%',data:pd.cm2,borderColor:col,backgroundColor:alpha,fill:false,tension:0.3,pointBackgroundColor:col,pointRadius:5,pointHoverRadius:7,borderDash:[5,3]}
    ]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:true,position:'top',labels:{font:{size:10},boxWidth:12}},tooltip:{callbacks:{label:function(ctx){return ' '+ctx.dataset.label+': '+ctx.parsed.y.toFixed(1)+'%';}}}},scales:{x:{grid:{display:false},ticks:{font:{size:10}}},y:{grid:{color:'#F0F5F5'},ticks:{font:{size:10},callback:function(v){return v+'%';}}}}}});
  });

  // Cross-portal grouped bars
  mkChart('c-gmv-cross',{type:'bar',data:{labels:crossLabels,datasets:portals.map(function(p,i){return{label:pLabels[p],data:gmvCross[i],backgroundColor:pColors[p]+'CC',borderColor:pColors[p],borderWidth:1,borderRadius:3};})},options:barOpts(false)});

  mkChart('c-mkt-cross',{type:'bar',data:{labels:crossLabels,datasets:portals.map(function(p,i){return{label:pLabels[p],data:adsvCross[i],backgroundColor:pColors[p]+'CC',borderColor:pColors[p],borderWidth:1,borderRadius:3};})},options:barOpts(true)});

  mkChart('c-cm1-cross',{type:'bar',data:{labels:crossLabels,datasets:portals.map(function(p,i){return{label:pLabels[p],data:cm1Cross[i],backgroundColor:pColors[p]+'CC',borderColor:pColors[p],borderWidth:1,borderRadius:3};})},options:barOpts(true)});

  mkChart('c-cm2-cross',{type:'bar',data:{labels:crossLabels,datasets:portals.map(function(p,i){return{label:pLabels[p],data:cm2Cross[i],backgroundColor:pColors[p]+'CC',borderColor:pColors[p],borderWidth:1,borderRadius:3};})},options:barOpts(true)});
