...
function initES() {
  ...
  if (S.month && S.data[dKey(S.portal,S.month)]) {
    ...
    // Clear any promos that leaked into NLC SKUs from old saves
    ES.nlcSkus.forEach(s => { s.promos = ''; });
...
function refreshCombinedTotal(){
  ...
  const cfg=S.config[S.portal];
  const reg=totals(ES.skus,   [], cfg, ES.portalTotals, null);
  const nlc=totals([], ES.nlcSkus, cfg, ES.portalTotals, ES.nlcTotals);
  ...
  el.innerHTML='<div class="card mb20" style="border:2px solid var(--green)">'
    ...
    +'<div><div style="font-size:10px;color:var(--tx3);font-weight:700;text-transform:uppercase;letter-spacing:.04em">Promos</div><div style="font-size:16px;font-weight:700;color:var(--warn)">₹'+fmt(promos)+'</div></div>'
    +'<div><div style="font-size:10px;color:var(--tx3);font-weight:700;text-transform:uppercase;letter-spacing:.04em">Ads + Visibility</div><div style="font-size:16px;font-weight:700;color:var(--warn)">₹'+fmt(ads+vis)+'</div></div>'
