...
function calcSKU(sku, cfg, isNLC, adsAlloc, visAlloc, promosAlloc) {
  ...
  if (sku.custom && (+sku.custom_netSales > 0 || +sku.custom_grossSales > 0)) {
    // Custom SKU: user types GMV, Gross Sales, Net Sales directly
...
const blankCustomSku = () => ({name:'',gmv:'',qty:'',cost:'',promos:'',ads:'',visibility:'',custom:true,custom_grossSales:'',custom_netSales:''});
...
function regRow(sku,i,cfg){
  ...
  // GMV / Gross Sales / Net Sales — editable for custom, auto-calc for regular
  const gmvCell = sku.custom
    ?'<input type="number" value="'+(sku.gmv||'')+'" placeholder="0" style="border-color:#4C94D044" oninput="updateReg('+i+',\'gmv\',this.value)">'
    :'<input type="number" value="'+(sku.gmv||'')+'" placeholder="0" oninput="updateReg('+i+',\'gmv\',this.value)">';
  const grossCell = sku.custom
    ?'<input type="number" value="'+(sku.custom_grossSales||'')+'" placeholder="0" style="background:#EFF6FF;border-color:#4C94D044" oninput="updateReg('+i+',\'custom_grossSales\',this.value)">'
    :'<input class="ro" type="text" value="'+(c.grossSales>0?'₹'+fmt(c.grossSales):'—')+'" readonly>';
  const netCell = sku.custom
    ?'<input type="number" value="'+(sku.custom_netSales||'')+'" placeholder="0" style="background:#EFF6FF;border-color:#4C94D044" oninput="updateReg('+i+',\'custom_netSales\',this.value)">'
    :'<input class="ro" type="text" value="'+(c.netSales>0?'₹'+fmt(c.netSales):'—')+'" readonly>';
...
function updateReg(i,f,v){ 
  ES.skus[i][f]=v; 
  // custom SKUs with editable gross/net sales need full row re-render to reflow calc
  if(ES.skus[i].custom && (f==='custom_grossSales'||f==='custom_netSales'||f==='gmv'||f==='qty'||f==='cost')){
    refreshReg();
  } else {
    _refreshRegRow(i);
  }
