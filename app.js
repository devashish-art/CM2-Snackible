...
function viewTrends(){
  ...
  // Gather all months across all portals
  const allMonths=[...new Set(MONTHS.filter(m=>portals.some(p=>S.data[dKey(p,m)])))];
  if(allMonths.length===0) return '<div class="ph"><div><div class="ph-title">Trends</div></div></div><div class="card empty"><div class="eicon">📈</div><p>No data yet.</p><button class="btn btn-yellow" onclick="go(\'entry\')">+ Enter Data</button></div>';

  // Build data matrix: for each portal, for each month
  const D={};
  portals.forEach(p=>{
    D[p]={gmv:[],netSales:[],adsvNet:[],adsvGmv:[],cm1Net:[],cm1Gmv:[],cm2Net:[],cm2Gmv:[],months:[]};
    allMonths.forEach(m=>{
      const e=S.data[dKey(p,m)];
      if(!e){return;}
      const t=totals(e.skus,e.nlcSkus,e.config||S.config[p],e.portalTotals,e.nlcTotals);
      D[p].months.push(m);
      D[p].gmv.push(Math.round(t.gmv));
      D[p].netSales.push(Math.round(t.netSales));
      D[p].adsvNet.push(t.netSales>0?+((t.ads+t.vis)/t.netSales*100).toFixed(2):0);
      D[p].adsvGmv.push(t.gmv>0?+((t.ads+t.vis)/t.gmv*100).toFixed(2):0);
      D[p].cm1Net.push(+t.cm1Pct.toFixed(2));
      D[p].cm1Gmv.push(t.gmv>0?+(t.cm1/t.gmv*100).toFixed(2):0);
      D[p].cm2Net.push(+t.cm2Pct.toFixed(2));
      D[p].cm2Gmv.push(t.gmv>0?+(t.cm2/t.gmv*100).toFixed(2):0);
    });
  });

  // Cross-portal: for each month, each portal's value (bar chart data)
  const crossLabels=allMonths.map(m=>m.replace(' 20',"'"));
  const crossGmv=portals.map(p=>allMonths.map(m=>{const e=S.data[dKey(p,m)];if(!e)return 0;const t=totals(e.skus,e.nlcSkus,e.config||S.config[p],e.portalTotals,e.nlcTotals);return Math.round(t.gmv);}));
  const crossAdsvNet=portals.map(p=>allMonths.map(m=>{const e=S.data[dKey(p,m)];if(!e)return null;const t=totals(e.skus,e.nlcSkus,e.config||S.config[p],e.portalTotals,e.nlcTotals);return t.netSales>0?+((t.ads+t.vis)/t.netSales*100).toFixed(2):null;}));
  const crossCm1Net=portals.map(p=>allMonths.map(m=>{const e=S.data[dKey(p,m)];if(!e)return null;const t=totals(e.skus,e.nlcSkus,e.config||S.config[p],e.portalTotals,e.nlcTotals);return +t.cm1Pct.toFixed(2);}));
  const crossCm2Net=portals.map(p=>allMonths.map(m=>{const e=S.data[dKey(p,m)];if(!e)return null;const t=totals(e.skus,e.nlcSkus,e.config||S.config[p],e.portalTotals,e.nlcTotals);return +t.cm2Pct.toFixed(2);}));

  const sectionHead=(title,sub)=>'<div style="margin:28px 0 14px"><div style="font-size:15px;font-weight:700;color:var(--tx)">'+title+'</div><div style="font-size:11px;color:var(--tx3);margin-top:2px">'+sub+'</div></div>';
  const chartCard=(title,id)=>'<div class="card" style="padding:16px"><div style="font-size:12px;font-weight:700;color:var(--tx);margin-bottom:10px">'+title+'</div><canvas id="'+id+'" height="180"></canvas></div>';
  const legend=ds=>'<div style="display:flex;gap:16px;margin-top:8px;flex-wrap:wrap">'+ds.map(d=>'<span style="font-size:10px;color:var(--tx3);display:flex;align-items:center;gap:4px"><span style="display:inline-block;width:10px;height:10px;background:'+d.color+';border-radius:50%"></span>'+d.label+'</span>').join('')+'</div>';
  const pLegend=legend(portals.map(p=>({color:pColors[p],label:pLabel(p)})));

  return '<div class="ph"><div><div class="ph-title">Trends</div><div class="ph-sub">All Portals · All Months</div></div></div>'

  // ── SECTION 1: GMV ──
  +sectionHead('1. GMV Trends','Portal GMV over time and cross-portal comparison')
  +'<div class="g2 mb20">'
  +portals.map(p=>D[p].months.length>0?chartCard(pLabel(p)+' GMV over months','c-gmv-'+p):'').join('')
  +'</div>'
  +'<div class="card mb20" style="padding:16px"><div style="font-size:12px;font-weight:700;color:var(--tx);margin-bottom:10px">GMV · All Portals Comparison</div><canvas id="c-gmv-cross" height="200"></canvas>'+pLegend+'</div>'

  // ── SECTION 2: Marketing % ──
  +sectionHead('2. Marketing Spends','Ads + Visibility as % of Net Sales — over time and cross-portal')
  +'<div class="g2 mb20">'
  +portals.map(p=>D[p].months.length>0?chartCard(pLabel(p)+' Ads+Vis % of Net Sales','c-mkt-'+p):'').join('')
  +'</div>'
  +'<div class="card mb20" style="padding:16px"><div style="font-size:12px;font-weight:700;color:var(--tx);margin-bottom:10px">Ads+Vis % Net Sales · All Portals Comparison</div><canvas id="c-mkt-cross" height="200"></canvas>'+pLegend+'</div>'

  // ── SECTION 3: CM1 & CM2 % ──
  +sectionHead('3. CM1 & CM2 Margins','As % of Net Sales — over time and cross-portal')
  +'<div class="g2 mb20">'
  +portals.map(p=>D[p].months.length>0?chartCard(pLabel(p)+' CM1% & CM2% of Net Sales','c-cm-'+p):'').join('')
  +'</div>'
  +'<div class="g2 mb20">'
  +'<div class="card" style="padding:16px"><div style="font-size:12px;font-weight:700;color:var(--tx);margin-bottom:10px">CM1% Net Sales · All Portals</div><canvas id="c-cm1-cross" height="200"></canvas>'+pLegend+'</div>'
  +'<div class="card" style="padding:16px"><div style="font-size:12px;font-weight:700;color:var(--tx);margin-bottom:10px">CM2% Net Sales · All Portals</div><canvas id="c-cm2-cross" height="200"></canvas>'+pLegend+'</div>'
  +'</div>'
  ...
  +'var crossLabels='+JSON.stringify(crossLabels)+';\n'
  +'var crossGmv='+JSON.stringify(crossGmv)+';\n'
  +'var crossAdsvNet='+JSON.stringify(crossAdsvNet)+';\n'
  +'var crossCm1Net='+JSON.stringify(crossCm1Net)+';\n'
  +'var crossCm2Net='+JSON.stringify(crossCm2Net)+';\n'

  // bar chart helper
  +'function barGroup(id,labs,datasets,isPct){'
  +'var cv=document.getElementById(id);if(!cv)return;'
  +'var W=cv.offsetWidth||600;cv.width=W;var H=cv.height||200;'
  +'var ctx=cv.getContext("2d");'
  +'var p={t:10,r:10,b:30,l:isPct?40:56};'
  +'var cw=W-p.l-p.r,ch=H-p.t-p.b,n=labs.length,nd=datasets.length;'
  +'var allVals=datasets.reduce(function(a,d){return a.concat(d.data.filter(function(v){return v!==null;}));},[] );'
  +'var maxV=Math.max.apply(null,allVals.map(Math.abs).concat([1]));'
  +'var bw=Math.max(6,Math.floor((cw/n)*0.8/nd));'
  +'var gap=Math.max(2,Math.floor((cw/n)*0.2));'
  +'ctx.clearRect(0,0,W,H);'
  +'for(var i=0;i<=4;i++){'
  +'var y=p.t+ch-(i/4)*ch;'
  +'ctx.strokeStyle="#F0F5F5";ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(p.l,y);ctx.lineTo(p.l+cw,y);ctx.stroke();'
  +'var v=maxV*(i/4);ctx.fillStyle="#8AA5A5";ctx.font="9px Poppins";ctx.textAlign="right";'
  +'ctx.fillText(isPct?v.toFixed(1)+"%":(v>=1e7?(v/1e7).toFixed(1)+"Cr":v>=1e5?(v/1e5).toFixed(1)+"L":v>=1e3?(v/1e3).toFixed(0)+"K":v.toFixed(0)),p.l-3,y+3);}'
  +'labs.forEach(function(lab,i){'
  +'var gx=p.l+i*(cw/n)+gap/2;'
  +'datasets.forEach(function(d,di){'
  +'if(d.data[i]===null)return;'
  +'var bh=Math.max(2,(Math.abs(d.data[i])/maxV)*ch);'
  +'var x=gx+di*(bw+2);'
  +'var y=d.data[i]>=0?p.t+ch-bh:p.t+ch;'
  +'ctx.fillStyle=d.color;ctx.fillRect(x,y,bw,bh);});'
  +'ctx.fillStyle="#0D1C1C";ctx.font="9px Poppins";ctx.textAlign="center";ctx.fillText(lab,p.l+i*(cw/n)+(cw/n)/2,p.t+ch+18);});}'

  // line chart helper
  +'function lineChart(id,labs,datasets,isPct){'
  +'var cv=document.getElementById(id);if(!cv)return;'
  +'var W=cv.offsetWidth||400;cv.width=W;var H=cv.height||180;'
  +'var ctx=cv.getContext("2d");'
  +'var p={t:10,r:10,b:28,l:isPct?38:52};'
  +'var cw=W-p.l-p.r,ch=H-p.t-p.b,n=labs.length;'
  +'var all=datasets.reduce(function(a,d){return a.concat(d.data);},[]);'
  +'var minV=Math.min.apply(null,all.concat([0])),maxV=Math.max.apply(null,all.concat([1]));'
  +'var rng=maxV-minV||1;'
  +'var fy=function(v){return p.t+ch-((v-minV)/rng)*ch;};'
  +'ctx.clearRect(0,0,W,H);'
  +'ctx.strokeStyle="#E8EEEE";ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(p.l,fy(0));ctx.lineTo(p.l+cw,fy(0));ctx.stroke();'
  +'for(var i=0;i<=4;i++){'
  +'var v=minV+(rng*(i/4));var y=fy(v);'
  +'ctx.strokeStyle="#F0F5F5";ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(p.l,y);ctx.lineTo(p.l+cw,y);ctx.stroke();'
  +'ctx.fillStyle="#8AA5A5";ctx.font="9px Poppins";ctx.textAlign="right";'
  +'ctx.fillText(isPct?v.toFixed(1)+"%":(v>=1e7?(v/1e7).toFixed(1)+"Cr":v>=1e5?(v/1e5).toFixed(1)+"L":v>=1e3?(v/1e3).toFixed(0)+"K":v.toFixed(0)),p.l-3,y+3);}'
  ...
  +'ctx.fillStyle=d.color;ctx.beginPath();ctx.arc(pt.x,pt.y,4,0,Math.PI*2);ctx.fill();'
  +'ctx.fillStyle="#fff";ctx.beginPath();ctx.arc(pt.x,pt.y,2,0,Math.PI*2);ctx.fill();'
  +'var lbl=isPct?d.data[i].toFixed(1)+"%":(d.data[i]>=1e7?(d.data[i]/1e7).toFixed(1)+"Cr":d.data[i]>=1e5?(d.data[i]/1e5).toFixed(1)+"L":d.data[i]>=1e3?(d.data[i]/1e3).toFixed(0)+"K":d.data[i]);'
  +'ctx.fillStyle=d.color;ctx.font="8px Poppins";ctx.textAlign="center";ctx.fillText(lbl,pt.x,pt.y-8);});'
  ...
  +'setTimeout(function(){'
  // GMV single-portal lines
  +'portals.forEach(function(p){'
  +'if(!D[p].months.length)return;'
  +'var labs=D[p].months.map(function(m){return m.replace(" 20","\'");});'
  +'lineChart("c-gmv-"+p,labs,[{data:D[p].gmv,color:pColors[p]}],false);});'
  // GMV cross-portal bar
  +'barGroup("c-gmv-cross",crossLabels,portals.map(function(p,i){return{data:crossGmv[i],color:pColors[p],label:pLabels[p]};}),false);'
  // Marketing single-portal lines
  +'portals.forEach(function(p){'
  +'if(!D[p].months.length)return;'
  +'var labs=D[p].months.map(function(m){return m.replace(" 20","\'");});'
  +'lineChart("c-mkt-"+p,labs,[{data:D[p].adsvNet,color:pColors[p]}],true);});'
  // Marketing cross-portal bar
  +'barGroup("c-mkt-cross",crossLabels,portals.map(function(p,i){return{data:crossAdsvNet[i],color:pColors[p],label:pLabels[p]};}),true);'
  // CM single-portal lines (CM1 + CM2 together)
  +'portals.forEach(function(p){'
  +'if(!D[p].months.length)return;'
  +'var labs=D[p].months.map(function(m){return m.replace(" 20","\'");});'
  +'lineChart("c-cm-"+p,labs,[{data:D[p].cm1Net,color:"#4C94D0",label:"CM1%"},{data:D[p].cm2Net,color:pColors[p],label:"CM2%"}],true);});'
  // CM1 cross-portal bar
  +'barGroup("c-cm1-cross",crossLabels,portals.map(function(p,i){return{data:crossCm1Net[i],color:pColors[p],label:pLabels[p]};}),true);'
  // CM2 cross-portal bar
  +'barGroup("c-cm2-cross",crossLabels,portals.map(function(p,i){return{data:crossCm2Net[i],color:pColors[p],label:pLabels[p]};}),true);'
  +'},80);})();<\/script>';
}
