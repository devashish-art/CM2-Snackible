...
const blankCustomSku = () => ({name:'',gmv:'',qty:'',cost:'',promos:'',ads:'',visibility:'',custom:true});
...
function regRow(sku,i,cfg){
  ...
  // For custom SKUs: promos/ads/vis use typed values (not portal allocation)
  const promosPlaceholder = sku.custom ? '0' : (proAlloc>0?Math.round(proAlloc):'0');
  const adsPlaceholder    = sku.custom ? '0' : (adsAlloc>0?Math.round(adsAlloc):'0');
  const visPlaceholder    = sku.custom ? '0' : (visAlloc>0?Math.round(visAlloc):'0');
...
function _refreshCustomRow(i){
  ...
  const cfg=S.config[S.portal];
  const pt=ES.portalTotals;
  // For custom SKUs, promos/ads/vis are always typed — no portal allocation
  const c=calcSKU(sku,cfg,false,0,0,0);
  const row=document.querySelector('[data-ri="'+i+'"]'); if(!row)return;
  const ros=row.querySelectorAll('input.ro');
  // ro order: grossSales(0), netSales(1), cogs(2), directExp(3), grossMargin(4), labour(5), logistics(6), cm1(7), cm1%(8), cm2(9), cm2%(10)
  const fv=v=>v>0?'₹'+fmt(v):'—';
  if(ros[0])ros[0].value=fv(c.grossSales);
  if(ros[1])ros[1].value=fv(c.netSales);
  if(ros[2])ros[2].value=fv(c.cogs);
  if(ros[3])ros[3].value=fv(c.directExp);
  if(ros[4])ros[4].value=c.netSales>0?'₹'+fmt(c.grossMargin):'—';
  if(ros[5])ros[5].value=c.netSales>0?'₹'+fmt(c.labour):'—';
  if(ros[6])ros[6].value=c.netSales>0?'₹'+fmt(c.logistics):'—';
  if(ros[7])ros[7].value=c.netSales>0?'₹'+fmt(c.cm1):'—';
  if(ros[8]){ros[8].value=c.netSales>0?fmtPct(c.cm1Pct):'—';ros[8].className='ro '+pc(c.cm1Pct);}
  if(ros[9])ros[9].value=c.netSales>0?'₹'+fmt(c.cm2):'—';
