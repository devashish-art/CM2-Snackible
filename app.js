...
function viewTrends(){
  ...
  const fullCard=(title,id,h=280)=>`<div class="card mb16" style="padding:18px"><div style="font-size:12px;font-weight:700;color:var(--tx2);text-transform:uppercase;letter-spacing:.05em;margin-bottom:14px">${title}</div><div style="position:relative;height:${h}px"><canvas id="${id}"></canvas></div></div>`;
  ...
  function yScale(isPct){
    return{grid:{color:'#EBF2F2'},ticks:{font:{size:10},color:'#3A5A5A',callback:function(v){return isPct?v.toFixed(1)+'%':v.toFixed(1)+'L';}},grace:'12%'};
  }
  function xScale(rotate){
    return{grid:{display:false},ticks:{font:{size:10},color:'#3A5A5A',maxRotation:rotate||0,minRotation:0,autoSkip:true,maxTicksLimit:12}};
  }
    ...
    afterDatasetsDraw:function(chart){
      ...
      chart.data.datasets.forEach(function(ds,di){
        ...
        meta.data.forEach(function(el,idx){
          ...
          if(isBar){
            if(v>=0){ctx.textBaseline='bottom';ctx.fillText(label,el.x,el.y-3);}
            else{ctx.textBaseline='top';ctx.fillText(label,el.x,el.y+3);}
          } else {
            ctx.textBaseline='bottom';ctx.fillText(label,el.x,el.y-7);
          }
          ctx.restore();
        });
      });
  ...
  function lineOpts(isPct,unit){return{responsive:true,maintainAspectRatio:false,
    ...
    scales:{x:xScale(30),y:yScale(isPct)},_snackPct:isPct,_snackUnit:unit||'L'};}

  function barOpts(isPct,unit){return{responsive:true,maintainAspectRatio:false,
    plugins:{legend:{display:true,position:'top',labels:{font:{size:10},color:'#1A3A3A',boxWidth:10,padding:12}},
  ...
  function drawAll(){
    ...
    portals.forEach(function(p){
      ...
      ]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:true,position:'top',labels:{font:{size:10},color:'#1A3A3A',boxWidth:10}},tooltip:{callbacks:{label:function(ctx){return ' '+ctx.dataset.label+': '+ctx.parsed.y.toFixed(1)+'%';}}}},scales:{x:xScale(30),y:yScale(true)},_snackPct:true}});
    });
