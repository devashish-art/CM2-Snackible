...
function viewTrends(){
  ...
  // Short label: January 2026 → Jan'26
  ...
  const cCard=(title,id,h=240)=>`<div class="card" style="padding:18px;background:#1E2A35"><div style="font-size:12px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:.05em;margin-bottom:14px">${title}</div><div style="position:relative;height:${h}px"><canvas id="${id}"></canvas></div></div>`;
  const fullCard=(title,id,h=280)=>`<div class="card mb16" style="padding:18px;background:#1E2A35"><div style="font-size:12px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:.05em;margin-bottom:14px">${title}</div><div style="position:relative;height:${h}px"><canvas id="${id}"></canvas></div></div>`;
  ...
  const html=
    ...
    +'</div>'
    +fullCard('GMV Comparison · All Portals','c-gmv-cross',300)
    +pLeg

    +secHead(2,'Marketing Spends','Ads+Visibility & Promos as % of Net Sales or GMV')
    +'<div class="g3 mb20">'
    +portals.map(p=>cCard(pLabel(p)+' · Ads+Vis % NS','c-mkt-'+p,220)).join('')
    +'</div>'
    +'<div class="g3 mb20">'
    +portals.map(p=>cCard(pLabel(p)+' · Promos % NS'+(p==='instamart'?' (Reg SKUs)':''),'c-promo-'+p,220)).join('')
    +'</div>'
    +'<div class="card mb20" style="padding:18px;background:#1E2A35">'
    +'<div style="font-size:12px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:.05em;margin-bottom:10px">Marketing Spends · All Portals</div>'
    +mktToggle
    +fullCard('Ads + Visibility · All Portals','c-adsv-cross',280)
    +fullCard('Promos · All Portals','c-promos-cross',280)
    +pLeg+'</div>'
    ...
    +'</div>'
    +fullCard('CM1% Net Sales · All Portals','c-cm1-cross',280)
    +pLeg
    +fullCard('CM2% Net Sales · All Portals','c-cm2-cross',280)
    +pLeg;
  ...
  var perPortalFull=${JSON.stringify(Object.fromEntries(portals.map(p=>[p,{
  ...
  var gmvCrossFull=${JSON.stringify(D.gmvCross)}.map(function(arr){return arr.map(function(v){return v===null?null:+(v/100000).toFixed(1);});});
  var adsvNetCrossFull=${JSON.stringify(D.adsvNetCross)};
  var adsvGmvCrossFull=${JSON.stringify(D.adsvGmvCross)};
  var promosNetCrossFull=${JSON.stringify(D.promosNetCross)};
  var promosGmvCrossFull=${JSON.stringify(D.promosGmvCross)};
  var cm1CrossFull=${JSON.stringify(D.cm1Cross)};
  var cm2CrossFull=${JSON.stringify(D.cm2Cross)};
  ...
  function mkChart(id,cfg){destroy(id);var cv=document.getElementById(id);if(!cv)return;charts[id]=new Chart(cv,cfg);if(cfg.options){charts[id]._snackPct=cfg.options._snackPct;charts[id]._snackUnit=cfg.options._snackUnit;}}
    ...
    afterDatasetsDraw:function(chart){
      ...
      chart.data.datasets.forEach(function(ds,di){
        ...
        meta.data.forEach(function(el,idx){
          ...
          ctx.save();
          // White background pill behind label for contrast
          ctx.font='bold 10px Poppins,sans-serif';
          var tw=ctx.measureText(label).width;
          var px=el.x, py;
          if(isBar){
            py=v>=0?el.y-4:el.y+14;
          } else {
            py=el.y-10;
          }
          // Draw white background pill with border
          ctx.fillStyle='rgba(30,42,53,0.92)';
          ctx.strokeStyle='rgba(255,255,255,0.15)';
          ctx.lineWidth=0.5;
          var rx=px-tw/2-3, ry=py-11, rw=tw+6, rh=14;
          ctx.beginPath();
  ...
  // Dark chart defaults
  var DARK_BG='#1E2A35';
  var AXIS_COLOR='#CBD5E1';
  var GRID_COLOR='rgba(255,255,255,0.08)';
  var LABEL_COLOR='#F1F5F9';

  function yScale(isPct){
    return{grid:{color:GRID_COLOR},ticks:{font:{size:11},color:AXIS_COLOR,callback:function(v){return isPct?v.toFixed(1)+'%':v.toFixed(1)+'L';}},grace:'12%'};
  }
  function xScale(rotate){
    return{grid:{display:false},ticks:{font:{size:11},color:AXIS_COLOR,maxRotation:rotate||0,minRotation:0,autoSkip:true,maxTicksLimit:12}};
  }

  function lineOpts(isPct,unit){return{responsive:true,maintainAspectRatio:false,
    ...
    scales:{x:xScale(30),y:yScale(isPct)},
    _snackPct:isPct,_snackUnit:unit||'L'};}

  function barOpts(isPct,unit){return{responsive:true,maintainAspectRatio:false,
    interaction:{mode:'index',intersect:false},
    plugins:{legend:{display:true,position:'top',labels:{font:{size:11},color:LABEL_COLOR,boxWidth:10,padding:12}},
      tooltip:{callbacks:{label:function(ctx){return ' '+ctx.dataset.label+': '+(isPct?ctx.parsed.y.toFixed(1)+'%':ctx.parsed.y.toFixed(1)+(unit||'L'));}}}},
    scales:{x:xScale(30),y:yScale(isPct)},
    barPercentage:0.6,categoryPercentage:0.7,_snackPct:isPct,_snackUnit:unit||'L'};}

  // Slice cross data by range
  function sliceCross(arr){return arr.map(function(d){return d.slice(rangeFrom,rangeTo+1);});}
  function sliceLabels(){return allLabels.slice(rangeFrom,rangeTo+1);}

  // Slice per-portal data by filtering months within range
  function slicePortal(p,field){
    var pd=perPortalFull[p];
    var result=[],months=[];
    pd.months.forEach(function(m,i){
      // find index in allLabels
      var ai=allLabels.indexOf(m);
      if(ai>=rangeFrom&&ai<=rangeTo){result.push(pd[field][i]);months.push(m);}
    });
    return{data:result,months:months};
  }

  function drawAll(){
    var labels=sliceLabels();

    portals.forEach(function(p){
      var col=pColors[p];
      var gmv=slicePortal(p,'gmv');
      var mkt=slicePortal(p,'adsvNet');
      var promo=slicePortal(p,'promosNet');
      var cm1=slicePortal(p,'cm1');
      var cm2=slicePortal(p,'cm2');
      if(!gmv.months.length)return;

      mkChart('c-gmv-'+p,{type:'line',data:{labels:gmv.months,datasets:[{label:'GMV (L)',data:gmv.data,borderColor:col,backgroundColor:col+'22',fill:true,tension:0.35,pointBackgroundColor:col,pointRadius:4,pointHoverRadius:6,borderWidth:2.5}]},options:lineOpts(false,'L')});
      mkChart('c-mkt-'+p,{type:'line',data:{labels:mkt.months,datasets:[{label:'Ads+Vis %',data:mkt.data,borderColor:col,backgroundColor:col+'22',fill:true,tension:0.35,pointBackgroundColor:col,pointRadius:4,pointHoverRadius:6,borderWidth:2.5}]},options:lineOpts(true)});
      mkChart('c-promo-'+p,{type:'line',data:{labels:promo.months,datasets:[{label:'Promos %',data:promo.data,borderColor:col,backgroundColor:col+'22',fill:true,tension:0.35,pointBackgroundColor:col,pointRadius:4,pointHoverRadius:6,borderWidth:2.5}]},options:lineOpts(true)});
      mkChart('c-cm-'+p,{type:'line',data:{labels:cm1.months,datasets:[
        {label:'CM1%',data:cm1.data,borderColor:'#4C94D0',backgroundColor:'#4C94D011',fill:false,tension:0.35,pointBackgroundColor:'#4C94D0',pointRadius:4,pointHoverRadius:6,borderWidth:2.5},
        {label:'CM2%',data:cm2.data,borderColor:col,backgroundColor:col+'11',fill:false,tension:0.35,pointBackgroundColor:col,pointRadius:4,pointHoverRadius:6,borderWidth:2.5,borderDash:[5,3]}
      ]},options:{responsive:true,maintainAspectRatio:false,interaction:{mode:'index',intersect:false},plugins:{legend:{display:true,position:'top',labels:{font:{size:11},color:LABEL_COLOR,boxWidth:10}},tooltip:{callbacks:{label:function(ctx){return ' '+ctx.dataset.label+': '+ctx.parsed.y.toFixed(1)+'%';}}}},scales:{x:xScale(30),y:yScale(true)},_snackPct:true}});
    });

    var gmvC=sliceCross(gmvCrossFull);
    mkChart('c-gmv-cross',{type:'bar',data:{labels:labels,datasets:portals.map(function(p,i){return{label:pLabels[p],data:gmvC[i],backgroundColor:pColors[p]+'DD',borderColor:pColors[p],borderWidth:1,borderRadius:4};})},options:barOpts(false,'L')});
    mkChart('c-cm1-cross',{type:'bar',data:{labels:labels,datasets:portals.map(function(p,i){return{label:pLabels[p],data:sliceCross(cm1CrossFull)[i],backgroundColor:pColors[p]+'DD',borderColor:pColors[p],borderWidth:1,borderRadius:4};})},options:barOpts(true)});
    mkChart('c-cm2-cross',{type:'bar',data:{labels:labels,datasets:portals.map(function(p,i){return{label:pLabels[p],data:sliceCross(cm2CrossFull)[i],backgroundColor:pColors[p]+'DD',borderColor:pColors[p],borderWidth:1,borderRadius:4};})},options:barOpts(true)});
    drawMktCharts();
  }
  ...
  function drawMktCharts(){
    ...
    var adsvData=sliceCross(isGmv?adsvGmvCrossFull:adsvNetCrossFull);
    var promosData=sliceCross(isGmv?promosGmvCrossFull:promosNetCrossFull);
    mkChart('c-adsv-cross',{type:'bar',data:{labels:labels,datasets:portals.map(function(p,i){return{label:pLabels[p],data:adsvData[i],backgroundColor:pColors[p]+'DD',borderColor:pColors[p],borderWidth:1,borderRadius:4};})},options:barOpts(true)});
    mkChart('c-promos-cross',{type:'bar',data:{labels:labels,datasets:portals.map(function(p,i){return{label:pLabels[p],data:promosData[i],backgroundColor:pColors[p]+'DD',borderColor:pColors[p],borderWidth:1,borderRadius:4};})},options:barOpts(true)});
  }
