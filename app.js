...
function viewCombined(){
  ...
  const pCards=portals.map(p=>{const t=tMap[p];const cm2gmv=t&&t.gmv>0?t.cm2/t.gmv*100:0;return t?'<div class="card stat"><div class="lbl">'+pLabel(p)+'</div><div class="val" style="color:'+pColor(p)+';font-size:18px">₹'+fmt(t.gmv)+'</div><div class="sub">CM1 '+fmtPct(t.cm1Pct)+' · CM2 <span style="color:'+(t.cm2Pct>=0?'var(--pos)':'var(--neg)')+'">'+fmtPct(t.cm2Pct)+'</span> · CM2/GMV <span style="color:'+(cm2gmv>=0?'var(--pos)':'var(--neg)')+'">'+fmtPct(cm2gmv)+'</span></div><div class="abar" style="background:'+pColor(p)+'"></div></div>':'<div class="card stat"><div class="lbl">'+pLabel(p)+'</div><div class="val" style="font-size:15px;color:var(--tx3)">No data</div><div class="sub"><button class="btn btn-outline btn-sm" onclick="setPortal(\''+p+'\');go(\'entry\')">+ Enter</button></div></div>';}).join('');
  const cmpRows=portals.map(p=>{
    const t=tMap[p];
    if(!t)return '<tr><td>'+pBadge(p)+'</td><td colspan="8" style="color:var(--tx3)">No data for '+sel+'</td></tr>';
    const cm2gmv=t.gmv>0?t.cm2/t.gmv*100:0;
    return '<tr><td>'+pBadge(p)+'</td>'
      +'<td class="r">₹'+fmt(t.gmv)+'</td>'
      +'<td class="r">₹'+fmt(t.netSales)+'</td>'
      +'<td class="r">₹'+fmt(t.grossMargin)+'</td>'
      +'<td class="r">₹'+fmt(t.cm1)+' <span class="pill '+pc(t.cm1Pct)+'" style="font-size:10px">'+fmtPct(t.cm1Pct)+'</span></td>'
      +'<td class="r">₹'+fmt(t.promos+t.ads+t.vis)+'</td>'
      +'<td class="r">₹'+fmt(t.cm2)+'</td>'
