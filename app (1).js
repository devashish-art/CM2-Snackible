...
// Pass portalTotals = {ads, vis, promos} for portal-level allocation; pass null to skip
function totals(skus, nlcSkus, cfg, portalTotals) {
  ...
  // First pass: compute raw netSales per SKU to get proportionate weights
  const allSkus = [...(skus||[]).map(s=>({s,n:false})), ...(nlcSkus||[]).map(s=>({s,n:true}))];
  const rawNS   = allSkus.map(({s,n})=>calcSKU(s,cfg,n,0,0,0).netSales);
  const totalNS = rawNS.reduce((a,v)=>a+v,0);
  ...
  allSkus.forEach(({s,n},idx)=>{
    const share    = totalNS > 0 ? rawNS[idx]/totalNS : 0;
    // Only use portal allocation if SKU has no override
...
function initES() {
  ...
  if (S.month && S.data[dKey(S.portal,S.month)]) {
    ...
    ES.portalTotals = JSON.parse(JSON.stringify(ex.portalTotals ||{ads:'',vis:'',promos:''}));
    ES.month        = S.month;
  } else {
    ES.skus=[]; ES.nlcSkus=[];
    ES.portalTotals = {ads:'',vis:'',promos:''};
...
function viewEntry() {
  ...
  const regTbl = '<div class="card tcard mb20">'
    ...
    +'<th class="r" style="min-width:100px">GMV (₹)</th>'
    +'<th class="r" style="min-width:105px">Gross Sales</th>'
    +'<th class="r" style="min-width:100px">Net Sales</th>'
    +'<th class="r" style="min-width:65px">Qty</th>'
    +'<th class="r" style="min-width:85px">Cost/Unit</th>'
    +'<th class="r" style="min-width:90px">COGS</th>'
    +'<th class="r" style="min-width:90px">Direct Exp</th>'
    +'<th class="r" style="min-width:105px">Gross Margin</th>'
    +'<th class="r" style="min-width:80px">Labour</th>'
    +'<th class="r" style="min-width:85px">Logistics</th>'
    +'<th class="r" style="min-width:85px">CM1 ₹</th>'
    +'<th class="r" style="min-width:60px">CM1%</th>'
    +'<th class="r" style="min-width:90px">Promos (₹)</th>'
    +'<th class="r" style="min-width:80px">Ads (₹)</th>'
    +'<th class="r" style="min-width:95px">Visibility (₹)</th>'
    +'<th class="r" style="min-width:85px">CM2 ₹</th>'
    +'<th class="r" style="min-width:60px">CM2%</th>'
    +'<th style="min-width:28px"></th></tr></thead>'
    +'<tbody id="reg-body">'+(regRows||emptyRow(19,'Click + Add Row or Prefill All'))+'</tbody></table></div></div>';
...
function regRow(sku,i,cfg){
  const pt=ES.portalTotals;
  // compute proportionate allocation for this row based on current ES state
  const allRawNS=ES.skus.map(s=>calcSKU(s,cfg,false,0,0,0).netSales);
  const totalNS=allRawNS.reduce((a,v)=>a+v,0);
  const share=totalNS>0?allRawNS[i]/totalNS:0;
  ...
  return '<tr class="erow" data-ri="'+i+'">'
    ...
    +'<td><input type="number" value="'+(sku.promos||'')+'" placeholder="auto" oninput="updateReg('+i+',\'promos\',this.value)"></td>'
    +'<td><input type="number" value="'+(sku.ads||'')+'" placeholder="auto" oninput="updateReg('+i+',\'ads\',this.value)"></td>'
    +'<td><input type="number" value="'+(sku.visibility||'')+'" placeholder="auto" oninput="updateReg('+i+',\'visibility\',this.value)"></td>'
...
function refreshReg(){ const b=document.getElementById('reg-body'); if(!b)return; const cfg=S.config[S.portal]; b.innerHTML=ES.skus.length?ES.skus.map((s,i)=>regRow(s,i,cfg)).join(''):emptyRow(19,'Click + Add Row'); }
...
function _refreshRegRow(i){
  const pt=ES.portalTotals;
  const allRawNS=ES.skus.map(s=>calcSKU(s,S.config[S.portal],false,0,0,0).netSales);
  const totalNS=allRawNS.reduce((a,v)=>a+v,0);
  const share=totalNS>0?allRawNS[i]/totalNS:0;
  const sku=ES.skus[i];
  const adsAlloc   =(+sku.ads       ||0)>0?0:(+pt.ads   ||0)*share;
...
function updatePT(f,v){ ES.portalTotals[f]=parseFloat(v)||0; refreshReg(); }
...
const SHEET_IMPORT_URL = 'https://script.google.com/macros/s/AKfycbwxwkTH98ZOgR_rSKCUrV24EIdNa09hpIsyRZnO6NVZOlrhOmVeNY84MHkXxZSZcU3u/exec';
  ...
  try {
    ...
    rows.forEach(r => {
      ...
      if (existing) {
        ...
        ES.skus.push({ name:r.name, gmv:r.gmv||'', qty:r.qty||'', cost:r.cost||'', promos:'', ads:'', visibility:'' });
      }
    });
    refreshReg();
