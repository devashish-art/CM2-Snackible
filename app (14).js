...
function viewDashboard() {
  ...
  const rows=dAllSkus.map(({s:sku,n:isNLC},idx)=>{
    ...
    const aA=(+sku.ads       ||0)>0?0:pAds   *share;
    const vA=(+sku.visibility||0)>0?0:pVis   *share;
    const pA=(+sku.promos    ||0)>0?0:pPromos*share;
...
function nlcRow(sku,i,cfg){
  ...
  const myWeight=splitBy==='qty'?(allRaw[i]?.qty||0):(allRaw[i]?.netSales||0);
  const share=totalWeight>0?myWeight/totalWeight:0;
  const adsAlloc   =(+sku.ads       ||0)>0?0:(+ES.nlcTotals.ads||0)*share;
  const visAlloc   =(+sku.visibility||0)>0?0:(+ES.nlcTotals.vis||0)*share;
  const proAlloc   =(+sku.promos    ||0)>0?0:(+pt.promos       ||0)*share;
