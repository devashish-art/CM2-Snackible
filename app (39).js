...
// ── SKU LEADERS ───────────────────────────────────────
function skuLeaders(dAllSkus, cfg, dTotalW, pAds, pVis, pPromos, dRawVals) {
  // Build per-SKU computed values (same allocation logic as rows)
  ...
  const medal  = ['🥇','🥈','🥉'];
  const gRow   = (x,i) => '<div class="sl-row">'
    +'<span class="sl-medal">'+medal[i]+'</span>'
    +'<span class="sl-name" title="'+x.name+'">'+x.name+(x.isNLC?' <span class="ntag">NLC</span>':'')+'</span>'
    +'<span class="sl-gmv">₹'+fmt(x.gmv)+'</span>'
    +'<span class="sl-pill '+pc(x.cm2Pct)+'">'+fmtPct(x.cm2Pct)+' CM2</span>'
    +'</div>';
  const cRow   = (x,i) => '<div class="sl-row">'
    +'<span class="sl-medal">'+medal[i]+'</span>'
    +'<span class="sl-name" title="'+x.name+'">'+x.name+(x.isNLC?' <span class="ntag">NLC</span>':'')+'</span>'
    +'<span class="sl-gmv '+pc(x.cm2Pct)+'">'+fmtPct(x.cm2Pct)+'</span>'
    +'<span class="sl-pill neutral">₹'+fmt(x.gmv)+' GMV</span>'
    +'</div>';

  return '<div class="g2 mb20">'
    +'<div class="card" style="padding:16px 20px"><div class="lbl" style="margin-bottom:10px;font-weight:700;font-size:12px;text-transform:uppercase;letter-spacing:.5px">🏆 Top 3 by GMV</div>'
    +(byGMV.length ? byGMV.map(gRow).join('') : '<div style="color:var(--tx3);font-size:13px">No data</div>')
    +'</div>'
    +'<div class="card" style="padding:16px 20px"><div class="lbl" style="margin-bottom:10px;font-weight:700;font-size:12px;text-transform:uppercase;letter-spacing:.5px">🚀 Top 3 by CM2%</div>'
    +(byCM2.length ? byCM2.map(cRow).join('') : '<div style="color:var(--tx3);font-size:13px">No data</div>')
    +'</div>'
    +'</div>';
}
...
function viewDashboard() {
  ...
  +skuLeaders(dAllSkus,cfg,dTotalW,pAds,pVis,pPromos,dRawVals)
...
function viewCombined(){
  ...
  const pCards=portals.map(p=>{
    ...
    return '<div class="card stat"><div class="lbl">'+pLabel(p)+'</div><div class="val" style="color:'+pColor(p)+';font-size:18px">₹'+fmt(t.gmv)+'</div>'+sub+'<div class="abar" style="background:'+pColor(p)+'"></div></div>';
  }).join('');
