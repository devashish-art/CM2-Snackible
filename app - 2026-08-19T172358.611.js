        ...
        options:{
          ...
          plugins:{legend:{display:true,position:'top',labels:{font:{size:11},color:'#FFFFFF',boxWidth:10,padding:12}},
            tooltip:{callbacks:{label:function(ctx){
              if(ctx.dataset.label==='Total GMV') return ' Total GMV: '+ctx.parsed.y.toFixed(1)+'L';
              return ' '+ctx.dataset.label+': '+ctx.parsed.y.toFixed(1)+'L';
            },footer:function(items){
              var total=items.filter(function(i){return i.dataset.label!=='Total GMV';}).reduce(function(s,i){return s+i.parsed.y;},0);
              return '';
            }}}},
          scales:{
