...
function viewTrends(){
  ...
  const html=
    ...
    +'<div style="margin:16px 0 12px;font-size:12px;color:var(--tx3);font-style:italic">↓ Cumulative CM2% — running weighted average as months accumulate</div>'
    +`<div class="card mb20" style="padding:18px;background:#1E2A35">
        <div style="font-size:12px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:.05em;margin-bottom:14px" id="cum-cm2-title">Cumulative CM2% · % of Net Sales</div>
  ...
  let tCm2=0,tNs=0,tGmv=0;
  const cumOverallNs=[], cumOverallGmv=[];
  allMonths.forEach(m=>{
    portals.forEach(p=>{const t=getTotals(p,m);if(t){tCm2+=t.cm2;tNs+=t.netSales;tGmv+=t.gmv;}});
    cumOverallNs.push(tNs>0?+(tCm2/tNs*100).toFixed(1):null);
    cumOverallGmv.push(tGmv>0?+(tCm2/tGmv*100).toFixed(1):null);
  });
  ...
  function drawCumulative(){
    ...
    var field=isGmv?'gv':'ns';
    var overallArr=isGmv?cumOverallGmv:cumOverallNs;
    var title=document.getElementById('cum-cm2-title');
    if(title)title.textContent='Cumulative CM2% · % of '+(isGmv?'GMV':'Net Sales');

    var datasets=portals.map(function(p){
      var data=allMonths.slice(rangeFrom,rangeTo+1).map(function(m,mi){
        return cumData[p][field][rangeFrom+mi]!==undefined?cumData[p][field][rangeFrom+mi]:null;
      });
      return{label:pLabels[p],data:data,borderColor:pColors[p],backgroundColor:'transparent',
        fill:false,tension:0.4,pointBackgroundColor:pColors[p],pointRadius:4,pointHoverRadius:6,borderWidth:2.5};
    });
    var overall=allMonths.slice(rangeFrom,rangeTo+1).map(function(m,mi){return overallArr[rangeFrom+mi];});
    datasets.push({label:'Overall',data:overall,borderColor:'#00E5FF',backgroundColor:'transparent',
      fill:false,tension:0.4,pointBackgroundColor:'#00E5FF',pointRadius:5,pointHoverRadius:7,borderWidth:3});

    destroy('c-cum-cm2');var cv=document.getElementById('c-cum-cm2');if(!cv)return;
    var opts=baseOpts(true,'%',true);
    opts.interaction={mode:'index',intersect:false};
    opts.plugins.legend={display:true,position:'top',labels:{font:{size:11},color:'#FFFFFF',boxWidth:10,padding:12}};
    opts.plugins.tooltip.callbacks.label=function(ctx){return ' '+ctx.dataset.label+': '+ctx.parsed.y.toFixed(1)+'%';};
