...
function viewCombined(){
  ...
  const pCards = portals.map(p => {
    ...
    const adsvTotalNet = t.netSales>0?(t.ads+t.vis)/t.netSales*100:0;
    const adsvTotalGmv = t.gmv>0?(t.ads+t.vis)/t.gmv*100:0;
    let sub = '';
    if (p==='instamart' && imReg) {
      const promosNetReg = imReg.netSales>0?imReg.promos/imReg.netSales*100:0;
      const promosGmvReg = imReg.gmv>0?imReg.promos/imReg.gmv*100:0;
      sub = 'Promos '+fmtPct(promosNetReg)+' reg-net / '+fmtPct(promosGmvReg)+' reg-GMV'
          + ' · A+V '+fmtPct(adsvTotalNet)+' net / '+fmtPct(adsvTotalGmv)+' GMV';
    } else {
      sub = 'A+V '+fmtPct(adsvTotalNet)+' net / '+fmtPct(adsvTotalGmv)+' GMV';
    }
