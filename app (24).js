...
function viewCombined(){
  ...
  const pCards=portals.map(p=>{const t=tMap[p];const cm1gmv=t&&t.gmv>0?t.cm1/t.gmv*100:0;const cm2gmv=t&&t.gmv>0?t.cm2/t.gmv*100:0;return t?'<div class="card stat"><div class="lbl">'+pLabel(p)+'</div><div class="val" style="color:'+pColor(p)+';font-size:18px">₹'+fmt(t.gmv)+'</div><div class="sub">CM1 '+fmtPct(t.cm1Pct)+' / '+fmtPct(cm1gmv)+' GMV · CM2 <span style="color:'+(t.cm2Pct>=0?'var(--pos)':'var(--neg)')+'">'+fmtPct(t.cm2Pct)+' / '+fmtPct(cm2gmv)+' GMV</span></div><div class="abar" style="background:'+pColor(p)+'"></div></div>':'<div class="card stat"><div class="lbl">'+pLabel(p)+'</div><div class="val" style="font-size:15px;color:var(--tx3)">No data</div><div class="sub"><button class="btn btn-outline btn-sm" onclick="setPortal(\''+p+'\');go(\'entry\')">+ Enter</button></div></div>';}).join('');
  const cmpRows=portals.map(p=>{
    const t=tMap[p];
    if(!t)return '<tr><td>'+pBadge(p)+'</td><td colspan="10" style="color:var(--tx3)">No data for '+sel+'</td></tr>';
  ...
  return '<div class="ph"><div><div class="ph-title">Combined View</div><div class="ph-sub">All Portals · '+sel+'</div></div><div class="ph-right"><span class="pbadge combined">🔀 All Portals</span> '+msel+'</div></div>'
    ...
    +'<div class="card tcard"><div class="thead-row"><div class="thead-title">Portal Comparison · '+sel+'</div></div><div class="twrap"><table><thead><tr><th>Portal</th><th class="r">GMV</th><th class="r">Net Sales</th><th class="r">Gross Margin</th><th class="r">CM1 ₹</th><th class="r">CM1% Net</th><th class="r">CM1% GMV</th><th class="r">Mktg Spend</th><th class="r">CM2 ₹</th><th class="r">CM2% Net</th><th class="r">CM2% GMV</th></tr></thead><tbody>'+cmpRows
