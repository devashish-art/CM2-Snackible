...
function viewTrends(){
  ...
  const html=
    ...
    +secHead(4,'Cumulative Performance','Running GMV and weighted CM2% as months accumulate')
    +`<div class="card mb20" style="padding:18px;background:#1E2A35">
        <div style="font-size:12px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:.05em;margin-bottom:14px">Cumulative GMV · All Portals (₹ Lakhs)</div>
        <div style="position:relative;height:280px"><canvas id="c-cum-gmv"></canvas></div>
      </div>`
    +`<div class="card mb20" style="padding:18px;background:#1E2A35">
        <div style="font-size:12px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:.05em;margin-bottom:14px">Cumulative CM2% · All Portals (Weighted Avg)</div>
        <div style="position:relative;height:280px"><canvas id="c-cum-cm2"></canvas></div>
      </div>`;
  ...
  function drawCumulative(){
    ...
    // Cumulative GMV per portal
    var cumGmvDatasets=portals.map(function(p){
      var cumSum=0;
      var data=allMonths.slice(rangeFrom,rangeTo+1).map(function(m){
        var d=allData[p];var i=d.months.indexOf(m);
        if(i>=0&&d.gmv[i]!==null) cumSum+=d.gmv[i];
        return cumSum>0?+cumSum.toFixed(1):null;
      });
      return{label:pLabels[p],data:data,borderColor:pColors[p],backgroundColor:pColors[p]+'33',
        fill:true,tension:0.35,pointBackgroundColor:pColors[p],pointRadius:3,pointHoverRadius:5,borderWidth:2.5};
    });

    // Total cumulative GMV line
    var cumTotalData=allMonths.slice(rangeFrom,rangeTo+1).map(function(m,mi){
      return cumGmvDatasets.reduce(function(sum,ds){return sum+(ds.data[mi]||0);},0);
    });
    cumGmvDatasets.push({
      label:'Total',data:cumTotalData,borderColor:'#00E5FF',backgroundColor:'transparent',
      fill:false,tension:0.35,pointBackgroundColor:'#00E5FF',pointRadius:4,pointHoverRadius:6,
      borderWidth:3,borderDash:[],order:0
    });

    destroy('c-cum-gmv');var cv1=document.getElementById('c-cum-gmv');if(cv1){
      var opts1=barOpts(false);
      opts1.interaction={mode:'index',intersect:false};
      opts1.plugins.legend={display:true,position:'top',labels:{font:{size:11},color:'#FFFFFF',boxWidth:10,padding:12}};
      opts1.plugins.tooltip.callbacks.label=function(ctx){return ' '+ctx.dataset.label+': '+ctx.parsed.y.toFixed(1)+'L';};
      charts['c-cum-gmv']=new Chart(cv1,{type:'line',data:{labels:labels,datasets:cumGmvDatasets},options:opts1});
      charts['c-cum-gmv']._snackPct=false;charts['c-cum-gmv']._snackUnit='L';
    }

    // Cumulative weighted CM2% per portal
    var cumCm2Datasets=portals.map(function(p){
      var cumCm2=0,cumNs=0;
      var data=allMonths.slice(rangeFrom,rangeTo+1).map(function(m){
        var e=S.data[dKey(p,m)];if(!e)return null;
        var t=totals(e.skus,e.nlcSkus,e.config||S.config[p],e.portalTotals,e.nlcTotals);
        cumCm2+=t.cm2;cumNs+=t.netSales;
        return cumNs>0?+(cumCm2/cumNs*100).toFixed(1):null;
      });
      return{label:pLabels[p],data:data,borderColor:pColors[p],backgroundColor:'transparent',
        fill:false,tension:0.35,pointBackgroundColor:pColors[p],pointRadius:4,pointHoverRadius:6,borderWidth:2.5};
    });

    // Overall weighted CM2%
    var cumTotalCm2=0,cumTotalNs=0;
    var cumOverallData=allMonths.slice(rangeFrom,rangeTo+1).map(function(m){
      portals.forEach(function(p){
        var e=S.data[dKey(p,m)];if(!e)return;
        var t=totals(e.skus,e.nlcSkus,e.config||S.config[p],e.portalTotals,e.nlcTotals);
        cumTotalCm2+=t.cm2;cumTotalNs+=t.netSales;
      });
      return cumTotalNs>0?+(cumTotalCm2/cumTotalNs*100).toFixed(1):null;
    });
    cumCm2Datasets.push({
      label:'Overall',data:cumOverallData,borderColor:'#00E5FF',backgroundColor:'transparent',
      fill:false,tension:0.35,pointBackgroundColor:'#00E5FF',pointRadius:4,pointHoverRadius:6,
      borderWidth:3,order:0
    });

    destroy('c-cum-cm2');var cv2=document.getElementById('c-cum-cm2');if(cv2){
      var opts2=baseOpts(true,'%',true);
      opts2.interaction={mode:'index',intersect:false};
      opts2.plugins.legend={display:true,position:'top',labels:{font:{size:11},color:'#FFFFFF',boxWidth:10,padding:12}};
      opts2.plugins.tooltip.callbacks.label=function(ctx){return ' '+ctx.dataset.label+': '+ctx.parsed.y.toFixed(1)+'%';};
      charts['c-cum-cm2']=new Chart(cv2,{type:'line',data:{labels:labels,datasets:cumCm2Datasets},options:opts2});
