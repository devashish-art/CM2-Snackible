...
function viewEntry() {
  ...
  const nlcTbl = !isIM ? '' :
    ...
    +'<th class="r" style="min-width:55px">CM2%</th>'
    +'<th style="min-width:24px"></th></tr></thead>'
    +'<tbody id="nlc-body">'+(nlcHtml||emptyRow(20,'Click + Add Row or Prefill All NLC SKUs'))+'</tbody></table></div></div>';
...
function refreshNlc(){ const b=document.getElementById('nlc-body'); if(!b)return; const cfg=S.config[S.portal]; b.innerHTML=ES.nlcSkus.length?ES.nlcSkus.map((s,i)=>nlcRow(s,i,cfg)).join(''):emptyRow(20,'Click + Add Row'); }
