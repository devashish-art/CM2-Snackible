...
function viewTrends(){
  ...
  function mkLine(id,labels,datasets,isPct,unit,showX,groupId){
    destroy(id);var cv=document.getElementById(id);if(!cv)return;
    var opts=baseOpts(isPct,unit,showX);
    opts.plugins.tooltip.enabled=false; // use custom tooltip
    opts.plugins.tooltip.external=function(){};
    charts[id]=new Chart(cv,{type:'line',data:{labels:labels,datasets:datasets},options:opts});
    charts[id]._snackPct=isPct;charts[id]._snackUnit=unit||'L';charts[id]._syncIdx=null;
    if(groupId){
