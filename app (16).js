...
// ── Calc ──────────────────────────────────────────────
// blank → use allocation; explicit 0 → zero; >0 → use value
const blankOrUndef = v => v===''||v===null||v===undefined;
...
function refreshCombinedTotal(){
  ...
  const cfg=S.config[S.portal];
  const t=totals(ES.skus,ES.nlcSkus,cfg,ES.portalTotals,ES.nlcTotals);
  if(!t||t.gmv===0){el.innerHTML='';return;}
  el.innerHTML='<div class="card mb20" style="border:2px solid var(--green)">'
    ...
    +'<div style="padding:12px 18px;display:flex;gap:32px;flex-wrap:wrap">'
    +'<div><div style="font-size:10px;color:var(--tx3);font-weight:700;text-transform:uppercase;letter-spacing:.04em">Total GMV</div><div style="font-size:16px;font-weight:700">₹'+fmt(t.gmv)+'</div></div>'
    +'<div><div style="font-size:10px;color:var(--tx3);font-weight:700;text-transform:uppercase;letter-spacing:.04em">Net Sales</div><div style="font-size:16px;font-weight:700">₹'+fmt(t.netSales)+'</div></div>'
    +'<div><div style="font-size:10px;color:var(--tx3);font-weight:700;text-transform:uppercase;letter-spacing:.04em">CM1 ₹</div><div style="font-size:16px;font-weight:700;color:var(--blue)">₹'+fmt(t.cm1)+'</div></div>'
    +'<div><div style="font-size:10px;color:var(--tx3);font-weight:700;text-transform:uppercase;letter-spacing:.04em">CM1%</div><div style="font-size:16px;font-weight:700;color:var(--blue)">'+fmtPct(t.cm1Pct)+'</div></div>'
    +'<div><div style="font-size:10px;color:var(--tx3);font-weight:700;text-transform:uppercase;letter-spacing:.04em">Ads + Visibility</div><div style="font-size:16px;font-weight:700;color:var(--warn)">₹'+fmt(t.ads+t.vis)+'</div></div>'
    +'<div><div style="font-size:10px;color:var(--tx3);font-weight:700;text-transform:uppercase;letter-spacing:.04em">CM2 ₹</div><div style="font-size:16px;font-weight:700;color:'+(t.cm2>=0?'var(--pos)':'var(--neg)')+'">₹'+fmt(t.cm2)+'</div></div>'
    +'<div><div style="font-size:10px;color:var(--tx3);font-weight:700;text-transform:uppercase;letter-spacing:.04em">CM2%</div><div style="font-size:16px;font-weight:700;color:'+(t.cm2Pct>=0?'var(--pos)':'var(--neg)')+'">'+fmtPct(t.cm2Pct)+'</div></div>'
    +'</div></div>';
}
...
function viewTrends(){
  ...
  months.forEach(m=>{ const e=S.data[dKey(S.portal,m)]||{}; const t=totals(e.skus,e.nlcSkus,e.config||S.config[S.portal],e.portalTotals,e.nlcTotals); gmvS.push(Math.round(t.gmv)); cm1S.push(+t.cm1Pct.toFixed(2)); cm2S.push(+t.cm2Pct.toFixed(2)); });
  ...
  const skuRows=lAllSkus.map(({s,n},idx)=>{
    ...
    const aA=blankOrUndef(s.ads)       ? (+lpt.ads   ||0)*share : 0;
    const vA=blankOrUndef(s.visibility)? (+lpt.vis   ||0)*share : 0;
    const pA=blankOrUndef(s.promos)    ? (+lpt.promos||0)*share : 0;
