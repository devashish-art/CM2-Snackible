...
function calcSKU(sku, cfg, isNLC, adsAlloc, visAlloc, promosAlloc) {
  ...
  if (sku.custom) {
    grossSales = +sku.custom_grossSales || 0;
    netSales   = +sku.custom_netSales   || 0;
  ...
  // For custom/NLC: use manually entered values if provided, else auto-calc from %
  const isManual = sku.custom || isNLC;
  const directExp  = isManual && sku.m_directExp  !== undefined && sku.m_directExp  !== '' ? +sku.m_directExp  : netSales * dePct;
  const labour     = isManual && sku.m_labour     !== undefined && sku.m_labour     !== '' ? +sku.m_labour     : netSales * labPct;
  const logistics  = isManual && sku.m_logistics  !== undefined && sku.m_logistics  !== '' ? +sku.m_logistics  : netSales * logPct;
...
function viewDashboard() {
  ...
  const regRows=dRegSkus.map(({s:sku},idx)=>{
    ...
    return '<tr>'
      +'<td style="max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="'+sku.name+'">'+shortN(sku.name)+'</td>'
...
function initES() {
  ...
  if (S.month && S.data[dKey(S.portal,S.month)]) {
    ...
    ES.skus         = JSON.parse(JSON.stringify(ex.skus ||[])).map(s=>({
      ...
      custom_grossSales: s.custom_grossSales||'',
      custom_netSales: s.custom_netSales||'',
      m_directExp: s.m_directExp!==undefined?s.m_directExp:'',
      m_labour: s.m_labour!==undefined?s.m_labour:'',
      m_logistics: s.m_logistics!==undefined?s.m_logistics:'',
      m_cogs: s.m_cogs!==undefined?s.m_cogs:'',
    }));
...
const blankCustomSku = () => ({name:'',gmv:'',qty:'',cost:'',promos:'',ads:'',visibility:'',custom:true,custom_grossSales:'',custom_netSales:'',m_directExp:'',m_labour:'',m_logistics:'',m_cogs:''});
const blankNlc = n => ({name:n||'',gmv:'',qty:'',nlc_price:'',cost:'',promos:'',ads:'',visibility:'',m_directExp:'',m_labour:'',m_logistics:''});
...
function regRow(sku,i,cfg){
  ...
  const es='style="background:#EFF6FF;border-color:#4C94D044"'; // editable style for custom
  const rowStyle=sku.custom?'style="background:#F0F7FF"':'';

  if(sku.custom){
    // All cells editable — no auto-calc, no re-render on keystroke
    return '<tr class="erow" data-ri="'+i+'" '+rowStyle+'>'
      +'<td>'+nameHtml+'</td>'
      +'<td><input type="number" value="'+(sku.gmv||'')+'" placeholder="0" '+es+' oninput="updateRegField('+i+',\'gmv\',this.value)"></td>'
      +'<td><input type="number" value="'+(sku.custom_grossSales||'')+'" placeholder="0" '+es+' oninput="updateRegField('+i+',\'custom_grossSales\',this.value)"></td>'
      +'<td><input type="number" value="'+(sku.custom_netSales||'')+'" placeholder="0" '+es+' oninput="updateRegField('+i+',\'custom_netSales\',this.value)"></td>'
      +'<td><input type="number" value="'+(sku.qty||'')+'" placeholder="0" oninput="updateRegField('+i+',\'qty\',this.value)"></td>'
      +'<td><input type="number" value="'+(sku.cost||'')+'" placeholder="0.00" step="0.01" oninput="updateRegField('+i+',\'cost\',this.value)"></td>'
      +'<td><input type="number" value="'+(sku.m_cogs!==undefined&&sku.m_cogs!==''?sku.m_cogs:'')+'" placeholder="'+(c.cogs>0?Math.round(c.cogs):0)+'" '+es+' oninput="updateRegField('+i+',\'m_cogs\',this.value)"></td>'
      +'<td><input type="number" value="'+(sku.m_directExp!==undefined&&sku.m_directExp!==''?sku.m_directExp:'')+'" placeholder="'+(c.directExp>0?Math.round(c.directExp):0)+'" '+es+' oninput="updateRegField('+i+',\'m_directExp\',this.value)"></td>'
      +'<td><input class="ro" type="text" value="'+(c.netSales>0?'₹'+fmt(c.grossMargin):'—')+'" readonly></td>'
      +'<td><input type="number" value="'+(sku.m_labour!==undefined&&sku.m_labour!==''?sku.m_labour:'')+'" placeholder="'+(c.labour>0?Math.round(c.labour):0)+'" '+es+' oninput="updateRegField('+i+',\'m_labour\',this.value)"></td>'
      +'<td><input type="number" value="'+(sku.m_logistics!==undefined&&sku.m_logistics!==''?sku.m_logistics:'')+'" placeholder="'+(c.logistics>0?Math.round(c.logistics):0)+'" '+es+' oninput="updateRegField('+i+',\'m_logistics\',this.value)"></td>'
      ...
      +'<td><input type="number" value="'+(sku.promos||'')+'" placeholder="0" oninput="updateRegField('+i+',\'promos\',this.value)"></td>'
      +'<td><input type="number" value="'+(sku.ads||'')+'" placeholder="0" oninput="updateRegField('+i+',\'ads\',this.value)"></td>'
      +'<td><input type="number" value="'+(sku.visibility||'')+'" placeholder="0" oninput="updateRegField('+i+',\'visibility\',this.value)"></td>'
  ...
  // Regular SKU — auto-calc as before
...
function nlcRow(sku,i,cfg){
  const c=calcSKU(sku,cfg,true,0,0,0);
  ...
  return '<tr class="erow" data-ni="'+i+'" style="background:#F5F9FF">'
    +'<td>'+selectHtml+'</td>'
    +'<td><input type="number" value="'+(sku.gmv||'')+'" placeholder="ref only" oninput="updateNlcField('+i+',\'gmv\',this.value)"></td>'
    +'<td><input type="number" value="'+(sku.nlc_price||'')+'" placeholder="NLC price" step="0.01" oninput="updateNlcField('+i+',\'nlc_price\',this.value)"></td>'
    +'<td><input type="number" value="'+(sku.qty||'')+'" placeholder="0" oninput="updateNlcField('+i+',\'qty\',this.value)"></td>'
    +'<td><input class="ro" type="text" value="'+(c.grossSales>0?'₹'+fmt(c.grossSales):'—')+'" readonly></td>'
    +'<td><input class="ro" type="text" value="'+(c.netSales>0?'₹'+fmt(c.netSales):'—')+'" readonly></td>'
    +'<td><input type="number" value="'+(sku.cost||'')+'" placeholder="0.00" step="0.01" oninput="updateNlcField('+i+',\'cost\',this.value)"></td>'
    +'<td><input class="ro" type="text" value="'+(c.cogs>0?'₹'+fmt(c.cogs):'—')+'" readonly></td>'
    +'<td><input type="number" value="'+(sku.m_directExp!==undefined&&sku.m_directExp!==''?sku.m_directExp:'')+'" placeholder="'+(c.directExp>0?Math.round(c.directExp):0)+'" '+es+' oninput="updateNlcField('+i+',\'m_directExp\',this.value)"></td>'
    +'<td><input class="ro" type="text" value="'+(c.netSales>0?'₹'+fmt(c.grossMargin):'—')+'" readonly></td>'
    +'<td><input type="number" value="'+(sku.m_labour!==undefined&&sku.m_labour!==''?sku.m_labour:'')+'" placeholder="'+(c.labour>0?Math.round(c.labour):0)+'" '+es+' oninput="updateNlcField('+i+',\'m_labour\',this.value)"></td>'
    +'<td><input type="number" value="'+(sku.m_logistics!==undefined&&sku.m_logistics!==''?sku.m_logistics:'')+'" placeholder="'+(c.logistics>0?Math.round(c.logistics):0)+'" '+es+' oninput="updateNlcField('+i+',\'m_logistics\',this.value)"></td>'
    +'<td><input class="ro" type="text" value="'+(c.netSales>0?'₹'+fmt(c.cm1):'—')+'" readonly></td>'
    +'<td><input class="ro '+pc(c.cm1Pct)+'" type="text" value="'+(c.netSales>0?fmtPct(c.cm1Pct):'—')+'" readonly></td>'
    +'<td><input type="number" value="'+(sku.promos||'')+'" placeholder="0" oninput="updateNlcField('+i+',\'promos\',this.value)"></td>'
    +'<td><input type="number" value="'+(sku.ads||'')+'" placeholder="0" oninput="updateNlcField('+i+',\'ads\',this.value)"></td>'
    +'<td><input type="number" value="'+(sku.visibility||'')+'" placeholder="0" oninput="updateNlcField('+i+',\'visibility\',this.value)"></td>'
    +'<td><input class="ro" type="text" value="'+(c.netSales>0?'₹'+fmt(c.cm2):'—')+'" readonly></td>'
    +'<td><input class="ro '+pc(c.cm2Pct)+'" type="text" value="'+(c.netSales>0?fmtPct(c.cm2Pct):'—')+'" readonly></td>'
...
function _refreshCustomRow(i){
  ...
  // custom row ro order: COGS, DirExp, GrMargin, Labour, Logistics, CM1, CM1%, CM2, CM2%
  ...
  if(ros[2])ros[2].value=fv(c.grossMargin);
  if(ros[3])ros[3].value=fv(c.labour);
  if(ros[4])ros[4].value=fv(c.logistics);
  if(ros[5])ros[5].value=fv(c.cm1);
  if(ros[6]){ros[6].value=c.netSales>0?fmtPct(c.cm1Pct):'—';ros[6].className='ro '+pc(c.cm1Pct);}
  if(ros[7])ros[7].value=fv(c.cm2);
...
function updateNlcField(i,f,v){
  // For NLC SKUs — update ES, refresh gross/net readonly cells only, then foot
  ES.nlcSkus[i][f]=v;
  const cfg=S.config[S.portal];
  const c=calcSKU(ES.nlcSkus[i],cfg,true,0,0,0);
  const row=document.querySelector('[data-ni="'+i+'"]'); 
  if(row){
    const ros=row.querySelectorAll('input.ro');
    const fv=v=>v>0?'₹'+fmt(v):'—';
    // ro order in nlcRow: grossSales(0), netSales(1), cogs(2), grossMargin(3), cm1(4), cm1%(5), cm2(6), cm2%(7)
    if(ros[0])ros[0].value=fv(c.grossSales);
    if(ros[1])ros[1].value=fv(c.netSales);
    if(ros[2])ros[2].value=fv(c.cogs);
    if(ros[3])ros[3].value=fv(c.grossMargin);
    if(ros[4])ros[4].value=fv(c.cm1);
    if(ros[5]){ros[5].value=c.netSales>0?fmtPct(c.cm1Pct):'—';ros[5].className='ro '+pc(c.cm1Pct);}
    if(ros[6])ros[6].value=fv(c.cm2);
    if(ros[7]){ros[7].value=c.netSales>0?fmtPct(c.cm2Pct):'—';ros[7].className='ro '+pc(c.cm2Pct);}
  }
  refreshNlcFoot();
}
...
function saveEntry(){
  ...
  const vs=ES.skus.filter(s=>s.name&&(+s.gmv>0||(s.custom&&(+s.custom_netSales>0||+s.custom_grossSales>0)))).map(s=>({
    ...s,
...
function viewTrends(){
  ...
  const html=
    ...
    +'</div>'
    +'<div class="g3 mb20">'
    +portals.map(p=>cCard(pLabel(p)+' · Promos % Net Sales'+(p==='instamart'?' (Reg SKUs)':''),'c-promo-'+p,240)).join('')
    +'</div>'
  ...
  var perPortal=${JSON.stringify(Object.fromEntries(portals.map(p=>[p,{
    ...
    promosNet:p==='instamart'
      ? portalMonths(p).map(m=>{const e=S.data[dKey(p,m)];if(!e)return 0;const reg=totals(e.skus,[],e.config||S.config[p],e.portalTotals,null);const full=getTotals(p,m);return reg.netSales>0?+((full?full.promos:0)/reg.netSales*100).toFixed(1):0;})
      : series(p,t=>t.netSales>0?+(t.promos/t.netSales*100).toFixed(1):0),
  }])))};
  ...
  Chart.register({
    ...
    afterDatasetsDraw:function(chart){
      ...
      chart.data.datasets.forEach(function(ds,di){
        ...
        var isPct=chart._snackPct;
        var unit=chart._snackUnit||'L';
        ...
        meta.data.forEach(function(el,idx){
          ...
          if(isBar){
            // positive: above bar; negative: below bar
            if(v>=0){ctx.textBaseline='bottom';ctx.fillText(label,el.x,el.y-2);}
            else{ctx.textBaseline='top';ctx.fillText(label,el.x,el.y+2);}
          } else {
            ctx.textBaseline='bottom';ctx.fillText(label,el.x,el.y-8);
          }
          ctx.restore();
        });
      });
  ...
  var lineBase=function(isPct,unit){return{responsive:true,maintainAspectRatio:false,
    plugins:{legend:{display:false},tooltip:{callbacks:{label:function(ctx){return ' '+(isPct?ctx.parsed.y.toFixed(1)+'%':ctx.parsed.y.toFixed(1)+(unit||'L'));}}}},
  ...
  // Patch mkChart to copy _snack options onto chart instance
  var _origMk=mkChart;
  mkChart=function(id,cfg){
    _origMk(id,cfg);
    if(charts[id]&&cfg.options){charts[id]._snackPct=cfg.options._snackPct;charts[id]._snackUnit=cfg.options._snackUnit;}
  };
  portals.forEach(function(p){
    ...
    mkChart('c-promo-'+p,{type:'line',data:{labels:pd.months,datasets:[{label:'Promos % NS',data:pd.promosNet,borderColor:col,backgroundColor:col+'22',fill:true,tension:0.35,pointBackgroundColor:col,pointRadius:5,pointHoverRadius:7,borderWidth:2.5}]},options:lineBase(true)});
    ...
    ]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:true,position:'top',labels:{font:{size:10},boxWidth:10}},tooltip:{callbacks:{label:function(ctx){return ' '+ctx.dataset.label+': '+ctx.parsed.y.toFixed(1)+'%';}}}},scales:{x:{grid:{display:false},ticks:{font:{size:10}}},y:yScale(true)},_snackPct:true}});
  });
