...
function viewTrends(){
  ...
  const html=
    ...
    +mktToggle
    +multiPane('Marketing %',portals.map(p=>({id:'c-mkt-'+p,label:pLabel2(p),color:pColors[p]})),140)
    +`<div class="card mb20" style="padding:18px;background:#1E2A35">
        <div style="font-size:12px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:.05em;margin-bottom:14px">Ads+Visibility · All Portals</div>
        <div style="position:relative;height:260px"><canvas id="c-adsv-cross"></canvas></div>
      </div>`
    +`<div class="card mb20" style="padding:18px;background:#1E2A35">
        <div style="font-size:12px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:.05em;margin-bottom:14px">Promos · All Portals</div>
        <div style="position:relative;height:260px"><canvas id="c-promos-cross"></canvas></div>
      </div>`
  ...
  Chart.register({id:'snackDL',afterDatasetsDraw:function(chart){
    ...
    chart.data.datasets.forEach(function(ds,di){
      ...
      meta.data.forEach(function(el,idx){
        ...
        var px=el.x,py=isBar?(v>=0?el.y-4:el.y+17):el.y-11;
  ...
  function mkLine(id,labels,datasets,isPct,unit,showX){
    destroy(id);var cv=document.getElementById(id);if(!cv)return;
    var opts=baseOpts(isPct,unit,showX);
    opts.plugins.tooltip.callbacks.title=function(items){return items[0].label;};
    // Add all portals to tooltip for pane charts
    charts[id]=new Chart(cv,{type:'line',data:{labels:labels,datasets:datasets},options:opts});
    charts[id]._snackPct=isPct;charts[id]._snackUnit=unit||'L';
  }
  ...
  function drawAll(){
    ...
    portals.forEach(function(p,pi){
      ...
      mkLine('c-gmv-'+p,s.labels,[{label:pLabels[p],data:s.data,borderColor:col,backgroundColor:col+'22',
        ...
        false,'L',pi===portals.length-1);
    });
    var gc=sliceCross('gmv');mkBar('c-gmv-cross',gc.labels,gc.datasets,false);
  ...
  function drawMkt(){
    ...
    portals.forEach(function(p,pi){
      ...
      mkLine('c-mkt-'+p,av.labels,[
        {label:'Ads+Vis',data:av.data,borderColor:'#38BDF8',fill:false,tension:0.35,pointBackgroundColor:'#38BDF8',pointRadius:4,pointHoverRadius:6,borderWidth:2.5},
        {label:'Promos',data:pr.data,borderColor:col,fill:false,tension:0.35,pointBackgroundColor:col,pointRadius:4,pointHoverRadius:6,borderWidth:2.5,borderDash:[5,3]}
      ],true,'%',pi===portals.length-1);
    });
    var avc=sliceCross(avF);mkBar('c-adsv-cross',avc.labels,avc.datasets,true);
    var prc=sliceCross(prF);mkBar('c-promos-cross',prc.labels,prc.datasets,true);
  }
