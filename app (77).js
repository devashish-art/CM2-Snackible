      ...
      chart.data.datasets.forEach(function(ds,di){
        ...
        meta.data.forEach(function(el,idx){
          ...
          if(isBar){
            ...
            // Skip if too close to previous label (overlap prevention for line charts)
            if(!chart._lastLabelX) chart._lastLabelX={};
            var prevX=chart._lastLabelX[di]||-999;
            if(px-prevX < tw+8){ ctx.restore(); return; }
            chart._lastLabelX[di]=px;
            py=el.y-11;
          }
          ctx.fillStyle='#FFFFFF';
          ctx.strokeStyle='rgba(0,0,0,0.1)';
          ctx.lineWidth=0.5;
          var rx=px-tw/2-5, ry=py-13, rw=tw+10, rh=16;
          ctx.beginPath();
