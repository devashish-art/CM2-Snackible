...
async function save() {
  try {
    const payload = JSON.stringify({ config: S.config, data: S.data });
    const url = SHEET_IMPORT_URL + '?action=saveState&payload=' + encodeURIComponent(payload);
...
async function load() {
  try {
    const url = SHEET_IMPORT_URL + '?action=loadState';
...
const SHEET_IMPORT_URL = 'https://script.google.com/macros/s/AKfycbykOyGfS6I2s6RfPJ-yNogJyGvBpJE8AUlgFAhxY535KLmr4hcOmdQQKbfU4ExeNFlK/exec';
...
function saveEntry(){
  ...
  save().then(() => { S.month=month; toast('✅ '+pLabel(S.portal)+' · '+month+' saved'); go('dashboard'); });
}
...
function delMonth(p,m){ if(!confirm('Delete '+pLabel(p)+' · '+m+'?'))return; delete S.data[dKey(p,m)]; save().then(() => { S.month=null; toast('Deleted '+m); render(); }); }
